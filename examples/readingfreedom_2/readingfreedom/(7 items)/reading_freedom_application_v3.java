import processing.core.*; import java.applet.*; import java.awt.*; import java.awt.image.*; import java.awt.event.*; import java.io.*; import java.net.*; import java.text.*; import java.util.*; import java.util.zip.*; public class reading_freedom_application_v3 extends PApplet {// Ira Greenberg
...
// values that control rate at which article groups rotate (ranodmly defined for now)
float[] articleSpinTheta = new float[articleCount];
float[] articleSpinValue = new float[articleCount];
...
int[] chapterColors = { 
 ...
 0xff22AAFF, 0xff22AAFF, 0xff22AAFF, 0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,0xff22AAFF,
 ...
 0xff22FF88, 0xff22FF88, 0xff22FF88, 0xff22FF88,0xff22FF88,0xff22FF88,0xff22FF88,0xff22FF88};
...
float artPosX1, artPosY1;
float artPosX2, artPosY2;
float artRotX, artRotY;
float articleSpinDamping = .98f;

// control individual article group rotation
float articleSpin = 0;
...
public void setup(){
  ...
  for (int i=0; i<articleCount; i++){
    // instantiate article icosahedrons
    articles[i] = new Icosahedron(12);
    
    // fill article group rotations with random values
    articleSpinTheta[i] = 0;
    articleSpinValue[i] = random(-2*PI/180, 2*PI/180);
...
public void draw(){
  ...
  if (isRotateSafe){
    ...
    artPosY1 = round(mouseY-artPosY2);
    artPosX1 = round(mouseX-artPosX2);
  }
  // stop rotation after helix is born
  if (nodeCounter>articleCount-1 || isInitted){
    posY1 *= damping;
    posX1 *= damping;
    artPosX1 *= articleSpinDamping;
    artPosY1 *= articleSpinDamping;
    for (int i=0; i<nodeCounter; i++){
      articleSpinTheta[i] *= articleSpinDamping;
    }
  }
  rotY += posX1;
  rotX += -posY1;
  ...
  artRotX += -artPosY1;
   ...
   for (int i=0; i<nodeCounter; i++){
    articleSpinTheta[i] = articleSpinValue[i]*(artRotY+artRotX);
   }
  // custom rotation
...
public void drawNodes(){
  ...
  for (int i=0; i<nodeCounter; i++){
    ...
    for (int j=0; j<partCount[i]; j++){
     ...
     articleSpinTheta[i]+=articleSpinValue[i];
  }
...
public void calculateMainNodesLoc(){
  ...
  for (int i=0; i<articleCount; i++){
    az = cos(aAng)*aRad;
    ax = sin(aAng)*aRad;
   // az = cos(aAng+articleSpinTheta[i])*aRad;
   // ax = sin(aAng+articleSpinTheta[i])*aRad;
...
public void calculateSubNodesLoc(){
  ...
  for (int i=0; i<articleCount; i++){
    ...
    // they rotate upon birth
    float px=0, py=0, pz=0, pAng=0, pRad=partRadius;
    for (int j=0; j<partCount[i]; j++){
      px = articleLoc[i].x + cos(pAng+articleSpinTheta[i]) *  pRad;
      py = articleLoc[i].y + sin(pAng+articleSpinTheta[i]) *  pRad;
      pz = articleLoc[i].z;
  ...
  for (int i=0; i<nodeCounter; i++){
    ...
    if (isDragSafe){
      isRotateSafe = false;
       ...
       // articles[1].rotZ(frameCount*PI/200.0);
       // calculateSubNodesLoc();
...
public void mousePressed(){
  // stops auto rotating on birth if user clicks
  isInitted = true;
...
class Vector3D{
  ...
  static public void main(String args[]) {     PApplet.main(new String[] { "reading_freedom_application_v3" });  }}